...
希望系统回顾一下RL的基础知识，特别是对LLM领域比较重要的那些
...
即采样出来的轨迹的return最大，对这个目标求导数得到

$$
\nabla_{\theta}\mathcal{L}(\theta) = \sum_{\tau}{R(\tau)\nabla_{\theta}\pi_{\theta}(\tau)}
$$

这个式子本身没错，但它不能作为我们的损失函数，因为它“不方便用采样来算”

1. RL里我们拿不到所有轨迹 $\tau$，只能从当前策略采样一些轨迹。我们手里只有采样出来的轨迹，但是这个式子要的是所有的轨迹求和，现实中根本做不到。我们只能从 $\pi_{\theta}$中采样，因此最好写成期望的形式，得到采样的结果后直接用部分轨迹就能算整体的梯度了
2. `log_prob` 有很多更优良的性质
...
这就是轨迹级别的surrogate函数了，虽然和目标不完全一致，但是导数是一致的。
