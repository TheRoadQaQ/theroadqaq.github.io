...
因此可以不直接用return，而是用return 减去一个参考值，也就是baseline
...
但是 **$Q(s_t,a_t)$ 很难直接知道**，我们用采样完一次轨迹 $G_t$ 去近似 $Q(s_t,a_t)$
...
- $G_t$：某条具体轨迹上实际观察到的 future return
- $V(s_t)$：从状态 $s_t$ 出发、按当前策略继续走时的平均回报
- $Q(s_t,a_t)$：在状态 $s_t$ 先采取动作 $a_t$ 后的平均回报
- $A(s_t,a_t)=Q(s_t,a_t)-V(s_t)$：这个动作相对平均水平的优势
